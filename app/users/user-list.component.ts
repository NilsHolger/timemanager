...
export class UserListComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (users === null || users === undefined || users.length === 1){
            this.notificationService.printErrorMessage('Failed to load users.');
        } else {
        this.users = users;
        this.notificationService.printSuccessMessage('Success loading users');
        }
    }
    ...
    addUser() {
        ...
        var newUser = { id: -1, name: '', avatar: 'guest.jpg', profession: '', schedulesCreated: 0 };
